...
RIO_AX_weekly <- RIO_AX %>% tq_transmute(select = Price, mutate_fun = periodReturn, period = "weekly", col_rename = "Weekly Return" ) %>% left_join(RIO_AX) %>% relocate(Price, .after = Date )  %>% mutate(`Return lagged by 1 week` = lag(`Weekly Return`))

RIO_AX_monthly <- RIO_AX %>% tq_transmute(select = Price, mutate_fun = periodReturn, period = "monthly", col_rename = "Monthly Return" ) %>% left_join(RIO_AX) %>% relocate(Price, .after = Date ) %>% mutate(`Return lagged by 1 month` = lag(`Monthly Return`))
```
...
RET_regression_weekly <- lm(data = RIO_AX_weekly,`Weekly Return` ~ `Return lagged by 1 week`)
RET_regression_monthly <- lm(data = RIO_AX_monthly, `Monthly Return` ~ `Return lagged by 1 month`)
```


```{r plotting return vs return of previous day}
ggplot(data = RIO_AX_daily, aes(y= `Daily Return`, x = `Return lagged by 1 day`)) + geom_line() + ggtitle("Return on Rio Tinto shares, vs. Return of The Previous Day") + theme(plot.title = element_text(hjust = 0.5))

ggplot(data = RIO_AX_weekly, aes(y= `Weekly Return`, x = `Return lagged by 1 week`)) + geom_line() + ggtitle("Return on Rio Tinto shares, vs. Return of The Previous Week") + theme(plot.title = element_text(hjust = 0.5))

ggplot(data = RIO_AX_monthly, aes(y= `Monthly Return`, x = `Return lagged by 1 month`)) + geom_line() + ggtitle("Return on Rio Tinto shares, vs. Return of The Previous Month") + theme(plot.title = element_text(hjust = 0.5))
```
